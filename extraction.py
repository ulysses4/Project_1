...
def csv2html(df):
	"""converts df to html format"""

	HtmlStr = "<table>"

	HtmlStr += "<tr>"
	for col in range(0,len(df.columns)):
		HtmlStr += "<th>" + df.columns[col] + "</th>"
	HtmlStr += "</tr>"

	for index in range(0,len(df.index)):
		HtmlStr += "<tr>"
		for col in range(0,len(df.columns)):
			HtmlStr += "<td>" + str(df.iloc[index, col]) + "</td>"
		HtmlStr += "</tr>"
...
def format_col(df):
	...
	df["TIME"] = df["EXECUTION_TIME"].apply(lambda x: x.time().strftime("%H:%M:%S"))
...
def send_email(df_list, run_dt):
	...
	table_list = []
	table_name = ["Trades", "ASW", "Termination", "Novation", "Brokers"]

	for df in df_list:
		table_list.append(csv2html(df))

	text = "\n"
	html = "<html><body>\n"

	for i in range(0, len(table_list)):
		text += table_name[i].replace(".csv","").upper() +\
				"\n" + table_list[i] + "\n"

		html += """<p><font size="2", color="red">""" +\
				table_name[i].replace(".csv","").upper() +\
				"</font></p>\n" + table_list[i] + "<br>"
